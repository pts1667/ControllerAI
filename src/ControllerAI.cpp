...
controllerai::CControllerAI::CControllerAI(springai::OOAICallback* callback):
		callback(callback),
		skirmishAIId(callback != NULL ? callback->GetSkirmishAIId() : -1)
		{}

controllerai::CControllerAI::~CControllerAI() {}
...
int controllerai::CControllerAI::HandleEvent(int topic, const void* data) {
	...
	switch (topic) {
		case EVENT_UNIT_CREATED: {
			...
			std::string msgText = std::string("/say Hello Engine (from ControllerAI), num my units is ") + IntToString(friendlyUnits.size());
